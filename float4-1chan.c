...
int float4_1chan(struct core_param o, struct core_return *retstr) {
	...
	long double cur_mag[o.N], avg_mag[o.N];
	double window[o.N], pow_adj, dataz;
...
//printf("bps: %d\n",bps);
	...
	running = keepgoing = true;

	while ((ftello(istream) < dataend) && (keepgoing == true)) {
		nanosleep(&nsl,NULL);
		/*
