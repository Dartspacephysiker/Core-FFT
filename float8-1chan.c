...
int float8_1chan(struct core_param o, struct core_return *retstr) {
    ...
    long double *cur_mag, *avg_mag;
    double *window;
	double pow_adj, dataz;
...
//printf("bps: %d\n",bps);
	...
	if (!ret) {
		fprintf(stderr, "Failed to initialize FFTW threads!  Err %lu.\n", ret);
	}

	/*
	 * Memory allocation.
	 */
	printf("Memory allocation..."); fflush(stdout);

	samples = (double *) malloc(s_bytes);
    ...
    cur_mag = malloc(o.N * sizeof(long double));
	if (cur_mag == NULL) fprintf(stderr,"Failed to allocate current magnitude memory!\n");
    avg_mag = malloc(o.N * sizeof(long double));
	if (avg_mag == NULL) fprintf(stderr,"Failed to allocate average magnitude memory!\n");
    window = malloc(o.N * sizeof(double));
	if (window == NULL) fprintf(stderr,"Failed to allocate window memory!\n");

	printf("done.  (%liKB)\n",(s_bytes + i_bytes + o_bytes)/1024);  fflush(stdout);
	 ...
	 */
	window_sel(window,&o);
	pow_adj = 1/window_cog(&o)*2; //*(o.frequency/o.N);
	printf("pow_adj: %f\n",pow_adj);
	...
	while ((ftello(istream) < dataend) && running) {
		/*
...
//if (ffts == 247404) printf ("input %lu = %e\n",i,input[i]);
		}
...
//if (ffts == 247404) printf ("%lu in fft %lu is %Le (%e + I%e)\n",i,ffts,cur_mag[i],creal(output[i]),cimag(output[i]));
//if ((ffts >= 247400) && (ffts < 247420)) printf ("%lu in fft %lu for avg %lu/%lu is %Le\n",i,ffts,iavg,o.avg,cur_mag[i]);
/*			if (o.phases) {
		...
		if (iavg == o.avg) {
			 ...
			 */
			if (tstream != NULL) {
				ret = fscanf(tstream,"%lf\n",&time);
			}

			if (o.verbose) printf("fft %lu @ %f\n",ffts,time);
...
*/
			...
			for (i = bl_first-1; i < bl_last; i++) {
				if (o.linear == true) {
					...
					// log10(|x|^2)
					power = lrint(10 * log10(avg_mag[i]/o.avg));

					if (power > INT_MAX) {
						power = INT_MAX;
						printf("INT_MAX!\n");
					} else if (power < INT_MIN) {
						power = INT_MIN;
						printf("INT_MIN!, fft = %lu, avgmag = %Lf, curmag = %Lf\n",ffts,avg_mag[i],cur_mag[i]);
					}
				}
...
/*				if (o.phases) {
	...
	retstr->nfft = ffts;
	retstr->min[0] = (int) minmag;
	retstr->max[0] = (int) maxmag;
