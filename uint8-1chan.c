...
int uint8_1chan(struct core_param o, struct core_return *retstr) {
...
//	unsigned flags = FFTW_ESTIMATE|FFTW_DESTROY_INPUT;
	...
	double *input;
	fftw_complex *output;
	bps = sizeof(char); // real, short int data
	s_bytes = bps*o.N;
...
//printf("bps: %d\n",bps);
	...
	samples = (unsigned char *) malloc(s_bytes);
	...
	while ((ftello(istream) < dataend) && (running == true)) {
		...
		for (i = 0; i < o.N; i++) {
			...
			input[i] = dataz*window[i]; // Hanning window.
//			printf("%f ",input[i]);
		}
//		printf("\n"); fflush(stdout);
